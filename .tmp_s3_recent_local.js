...
(async () => {
  ...
  const items = (out.Contents || [])
    .map(x => ({ key: x.Key, lastModified: x.LastModified ? new Date(x.LastModified) : null, size: x.Size || 0 }))
    .sort((a, b) => (a.lastModified?.getTime() || 0) - (b.lastModified?.getTime() || 0));
  const last = items.slice(-10).map(i => ({
    key: i.key,
    utc: i.lastModified ? i.lastModified.toISOString() : null,
    local: i.lastModified ? i.lastModified.toLocaleString('hu-HU', { hour12: false }) : null,
    size: i.size,
  }));
  console.log(JSON.stringify({ count: items.length, last }, null, 2));
})();
